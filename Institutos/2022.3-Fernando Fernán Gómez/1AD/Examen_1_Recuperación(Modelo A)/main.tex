 %%%%%%%%%%%%%%%%%%%%%%%%%%%
 \newcommand{\documentName} { 2ª evaluación }
 \newcommand{\documentContent} { Fracciones y números decimales  } 
 \newcommand{\waterMark} { Modelo A } 
 %%%%%%%%%%%%%%%%%%%%%%%%%%%
 
 % Configuración del documento.
 \input{./include/configurationCurso} 
 \input{./include/configuration} 
 
 % \renewcommand{\thepartno}{\arabic{partno}}
 \usepackage{nicefrac, xfrac}
 
 %%%%%%%%%%%%%%%%%%%%%%%%%%%
 % Exam configuration
 %\pointsdroppedatright   %% No mostrar la puntuación
 \pointsinrightmargin{} % Para poner las puntuaciones a la derecha. Se puede cambiar. Si se comenta, sale a la izquierda.
 \extrawidth{-1.5cm} %Un poquito más de margen por si ponemos textos largos.
 \marginpointname{ \emph{\points}}
 
 %% Si se comenta no aparecerán los espacios de la solución.
 %\nocancelspace
 
 %% Esto es de la clase exam. Si dejamos sin comentar \printanswers, se mostraran las soluciones. 
 %% Si la comentamos y dejamos sin comentar \noprintanswers, pues no se muestran las soluciones.
 % \printanswers
 %\noprintanswers
 
 %%%%%%%%%%%%%%%%%%%%%%%%%%%
 
 \begin{document}
 
 \StudentData{}
 \GradeTableHeader{}
 
 \justifying{}
 
 \begin{questions}
    \question[1] Realiza las siguientes operaciones con números decimales:
        \begin {parts}
            \part $45,78 \cdot 1,9$
            \vspace{\stretch{1}}
            \part $\left(-1,6\right) \cdot \left(-3,4\right)$
            \vspace{\stretch{1}}
            \part $42 : 1,5$
            \vspace{\stretch{1}}
            \part $819,32+42,93-9145,13-48,12$
            \vspace{\stretch{1}}
        \end{parts}

    \question[1] Ordina los siguientes números de menor a mayor
        \begin {parts} 
            \part $\left\lbrace 45,22; 2,3442; -3,4; -45,22 \right\rbrace$
            \vspace{\stretch{1}}
            \part $\left\lbrace 5; 0,5; -5; 1,2 \right\rbrace$
            \vspace{\stretch{1}}
            \part $\left\lbrace -\frac{4}{18}, \frac{9}{4}, \frac{5}{3}, -\frac{3}{2} \right\rbrace$
            \vspace{\stretch{1}}
        \end{parts}

        \newpage{}
    \question[1] Realiza las siguientes operaciones con fracciones:
    \begin {parts} 
            \part $\frac{5}{6} + \frac{4}{5} - \frac{8}{10}$
            \vspace{\stretch{1}}
            \part $12 - \frac{7}{6}-6+\frac{8}{15}$
            \vspace{\stretch{1}}
            \part $-\left(\frac{2}{3} - \frac{1}{5}\right) - \left(\frac{7}{10} - \frac{1}{2}\right)$
            \vspace{\stretch{1}}
        \end{parts}
  \end{questions}
 
\end{document}