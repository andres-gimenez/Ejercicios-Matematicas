\documentclass[addpoints,spanish, 12pt,a4paper,cancelspace]{./include/gexam}

%%%%%%%%%%%%%%%%%%%%%%%%%%%
\renewcommand{\documentName} { 1ª evaluación 2º Examen  }
\renewcommand{\documentContent} { Fracciones } 
\renewcommand{\waterMark} { \phantom{Modelo A} } 

% Configuración del documento.
\input{./include/configuration} 
%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Exam configuration
%\pointsdroppedatright   %% No mostrar la puntuación
\pointsinrightmargin % Para poner las puntuaciones a la derecha. Se puede cambiar. Si se comenta, sale a la izquierda.
\extrawidth{-1.5cm} %Un poquito más de margen por si ponemos textos largos.
\marginpointname{ \emph{\points}}

%% Si se comenta no aparecerán los espacios de la solución.
%\nocancelspace

%% Esto es de la clase exam. Si dejamos sin comentar \printanswers, se mostraran las soluciones. 
%% Si la comentamos y dejamos sin comentar \noprintanswers, pues no se muestran las soluciones.
%\printanswers
%\noprintanswers

%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{document}

\StudentData{}
\GradeTableHeader{}

\justifying{}

% Examen para una clase de desdoble con muy poco nivel.

\begin{center}
    \fbox{\fbox{\parbox{6.5in}{             
                \begin{itemize}
                    \item Copiar, hablar, levantarse de la silla o molestar a al resto de la clase pueden ser motivos de la retirada del examen que se valorará con un cero.
                    \item Deben aparecer todas las operaciones, no vale solo con indicar el resultado.
                    \item Se ha de llegar a la solución más reducida posible.
                    % \item Se podrán quitar hasta cinco décimas por falta de claridad o rigor en el desarrollo de las respuestas o por una mala presentación.
                    % \item Se valorará que se indiquen las cuentas en línea, realizando las operaciones en el margen.
                    \item No está permitido utilizar la calculadora.
                \end{itemize}
            }}}
\end{center}

\begin{questions}
    \setcounter{question}{0}

    \question[2] Realiza las siguientes operaciones.
    \begin{parts}
        % \part $5 + 4 -3 + 8 -2 = $
        % \vspace{\stretch{1}}

        % \part $\left(+3\right) - \left(-4\right) - \left(+3\right) + \left(-8\right) - \left(+2\right) = $
        % \vspace{\stretch{1}}

        \part $\left(+2\right) - \left(-5\right) - \left(-2\right) + \left(+8\right) + \left(-2\right) =$
        \vspace{\stretch{1}}

        \part $\left(+6\right) + \left(-4\right) - \left(-6\right) + \left(+8\right) + \left(-4\right) =$
        \vspace{\stretch{1}}
    \end{parts}
    \newpage 

    \question[2] Opera y simplifica los resultados.
    \begin{parts}
        \part $\frac{5}{3} + \frac{4}{9} + \frac{4}{6} = $
        \vspace{\stretch{1}}
        
        \part $\frac{15}{18} + \frac{3}{16} - \frac{4}{9} = $
        \vspace{\stretch{1}}
    \end{parts}
    \newpage 

    \question[3] Resuelve y simplifica el resultado.
    \begin{parts}
        \part $\frac{4}{7} \cdot \frac{5}{3} =$ 
        \vspace{\stretch{1}}

        \part $\frac{3}{4} : \frac{5}{4} =$
        \vspace{\stretch{1}}

        \part $\frac{3}{4} \cdot \left( \frac{2}{3} + \frac{5}{4} \right) =$
        \vspace{\stretch{1}}
    \end{parts}
    \newpage 

    \question[3] Opera las siguientes operaciones y simplifica el resultado.
    \begin{parts}
        \part ${\left(\frac{4}{3}\right)}^2  + {\left(\frac{2}{3}\right)}^3= $ 
        \vspace{\stretch{1}}

        \part ${\left(\frac{1}{4} - \frac{7}{12}\right)}^{2} - {\left(\frac{2}{3} - 1\right)}^{2} =$
        \vspace{\stretch{1}}
    \end{parts}

\end{questions}
\end{document}