...
\renewcommand{\documentContent} { Expresiones algebraicas, polinomios y ecuaciones } 
...
\begin{questions}
    ...
    \question[2] Realiza y reduce al máximo la siguiente operación.
        \begin{parts}
    ...
    \question Resuelve las siguientes ecuaciones recuadrando el resultado.
    \begin{parts}
        % \part ${x-2 \over 10} - {x+3 \over 4} = {2x- 7 \over 8}$
        % \vspace{\stretch{1}}

        % \part[2] $\sqrt{x^2 +5} + 6 = 3-3x$
        \part[2] $2x-\sqrt{x-3} = 7$
        \vspace{\stretch{1}}

        % \part $2^x+2^{2x} = 6$
        % \vspace{\stretch{1}}

        \part[2] $\log_3{\left(2x^2-15\right)} = 4$
        \vspace{\stretch{1}}
        \newpage

        \part[2] $x^5 + 2 x^4 + 4 x^3 + 8 x^2 - 5 x - 10 = 0$
        \vspace{\stretch{1}}
    \end{parts}
