...
#g-question[Calcula el valor de los siguientes límites:
  ...
  #g-subquestion(point: 1.5)[$display(limits("lím")_(x->infinity) (3 - (4x) / (2x+1) )^(x+3))$]
  #v(5pt)


  #g-subquestion(point: 1)[$display(limits("lím")_(x->infinity) (sqrt(x-3) - x))$]
  #v(5pt)
...
#g-question(point: 1.5)[Determina $a$ y $b$ para que sea continua la función \
  #align(center, [
    $f(x) = display(cases( delim: "{",
                x^4 + 3x + 4        & #h(10pt) &"si" & #h(10pt) &      &x < 0,
                a dot "cos"(x) + b  &          &"si" &          & 0 <= &x <= pi/2,
                (x-pi/2)^2 + a      &          &"si" &          &      &x > pi/2
              ))$ 
   ]
  ) 
  // ¿La función que resulta, es derivable en $x=3$?
...
#g-question(point: 2)[Halla el plano que pasa por los puntos $A(0, 2, 0)$ y $B(2,0,1)$ y es perpendicular al plano $2x-2y-z=7$].

// Selectividad Madrid Junio-91
#g-question(point: 2)[Dado el plano $pi eq.triple 2x-y+2z-3=0$, hallar un punto $P$ de la recta  \
